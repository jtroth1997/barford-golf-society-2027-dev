(() => {
  ...
  async function setScorer(card,memberId){
    // Scores live on the group scorecard, not on the scorer account. Changing scorer therefore transfers
    // control of the SAME card and preserves every hole already entered.
    return client.rpc("select_scorecard_scorer",{target_event_id:card.event_id,target_scorer_id:memberId});
  }

  function showChangeChooser(box,state){
    const {session,card,players}=state;
    const current=players.find(p=>p.member_id===card.scorer_id);
    ...
    box.querySelectorAll("[data-transfer-scorer]").forEach(btn=>btn.addEventListener("click",async()=>{
      const chosen=players.find(p=>p.member_id===btn.dataset.transferScorer);
      if(!confirm(`Change scorer from ${current?.display_name||"the current scorer"} to ${chosen?.display_name||"this player"}? All scores already entered will be kept.`))return;
      box.querySelectorAll("button").forEach(b=>b.disabled=true);
      const status=document.getElementById("dashboardScorerStatus"); if(status)status.textContent="Transferring scorecard…";
      const {error}=await setScorer(card,btn.dataset.transferScorer);
  ...
  async function render(){
    ...
    try{
      ...
      if(card.status==="ready"&&!card.scorer_id){
        ...
        box.querySelectorAll("[data-pick-scorer]").forEach(btn=>btn.addEventListener("click",async()=>{const scrollY=window.scrollY;box.querySelectorAll("button").forEach(b=>b.disabled=true);const status=document.getElementById("dashboardScorerStatus");if(status)status.textContent="Saving scorer…";const {error}=await setScorer(card,btn.dataset.pickScorer);if(error){if(status)status.textContent=error.message;box.querySelectorAll("button").forEach(b=>b.disabled=false);return;}await render();requestAnimationFrame(()=>window.scrollTo({top:scrollY,behavior:"auto"}));}));
        return;
      }
      if(scorer&&["ready","in_progress"].includes(card.status)){
        const isMe=scorer.member_id===session.user.id;
        box.innerHTML=`<strong style="display:block">${isMe?"You’re keeping score":`${esc(scorer.display_name)} is keeping score`}</strong><small style="display:block;margin-bottom:10px">${isMe?"You can open the group card below.":"Only the nominated scorer can enter this group’s scores."}</small><button type="button" class="button button-outline" data-change-scorer>Change scorer</button>`;
        box.querySelector("[data-change-scorer]")?.addEventListener("click",()=>showChangeChooser(box,state));
        if(scoreLink)scoreLink.classList.toggle("hidden",!isMe);
        return;
      }
      box.innerHTML=`<strong>Scorecard submitted</strong><small>This group card is complete.</small>`;
    }finally{rendering=false;}
  }
  document.addEventListener("visibilitychange",()=>{if(!document.hidden)render().catch(console.error);});
  window.addEventListener("pageshow",()=>render().catch(console.error));
