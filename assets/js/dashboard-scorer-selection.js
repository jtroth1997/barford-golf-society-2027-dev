    ...
    try {
      ...
      if (card.status === "ready" && !card.scorer_id) {
        ...
        box.querySelectorAll("[data-pick-scorer]").forEach(btn => btn.addEventListener("click", async () => {
          ...
          const chosen=players.find(p=>p.member_id===btn.dataset.pickScorer);
          ...
          const isMe=btn.dataset.pickScorer===session.user.id;
          box.innerHTML=`<strong style="display:block">${isMe?"You’re keeping score":`${esc(chosen?.display_name||"Selected player")} is keeping score`}</strong><small>${isMe?"You can open the group card below.":"Only the nominated scorer can enter this group’s scores."}</small>`;
