(() => {
  ...
  const prepare=document.getElementById("adminPrepareScorecards");if(prepare){
    ...
    prepare.textContent="Post group scorecards";prepare.disabled=true;const groupStatus=document.createElement("p");groupStatus.id="adminGroupScorecardStatus";groupStatus.className="form-status";groupStatus.setAttribute("aria-live","polite");scorecardAction.append(prepare,progress,groupStatus);scorecardPanel.appendChild(scorecardAction);if(endRound&&resultsCard){const finishStatus=document.createElement("p");finishStatus.id="adminFinishRoundStatus";finishStatus.className="form-status";finishStatus.setAttribute("aria-live","polite");endRound.appendChild(finishStatus);resultsCard.appendChild(endRound);}
  }
