(() => {
  ...
  teeSelect?.addEventListener("change",async()=>{prepare.disabled=true;});
  publish?.addEventListener("click",async()=>{setTimeout(async()=>{const eventId=teeSelect?.value;if(!eventId)return;const{count}=await client.from("tee_times").select("id",{count:"exact",head:true}).eq("event_id",eventId);prepare.disabled=!(count>0);},500);},true);
