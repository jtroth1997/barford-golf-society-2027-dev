(() => {
  ...
  const loadEvents=async()=>{
    ...
    if(session){const [{data:rsvps},locks]=await Promise.all([client.from("rsvps").select("event_id,status").eq("member_id",session.user.id).in("event_id",events.map(e=>e.id)),Promise.all(events.map(e=>client.rpc("get_event_rsvp_lock_status",{target_event_id:e.id})))]);ownRsvps=rsvps||[];events.forEach((e,i)=>lockMap.set(e.id,Boolean(locks[i]?.data)));}
    const rsvpByEvent=new Map(ownRsvps.map(r=>[r.event_id,r]));list.innerHTML=events.map(e=>renderEvent(e,{signedIn:Boolean(session),rsvp:rsvpByEvent.get(e.id),locked:lockMap.get(e.id)})).join("");if(summary)summary.textContent=`${events.length} upcoming event${events.length===1?"":"s"} published.`;

    list.querySelectorAll("[data-event-rsvp]").forEach(button=>button.addEventListener("click",async()=>{
      if(!session){location.href="account.html";return;}const eventId=button.dataset.eventId,status=button.dataset.eventRsvp;button.disabled=true;const old=button.textContent;button.textContent="Saving…";
      const payload={event_id:eventId,member_id:session.user.id,status,updated_at:new Date().toISOString()};if(status==="playing"){payload.buggy_requested=false;payload.preferred_tee_time="dont_mind";}
      const {error:saveError}=await client.from("rsvps").upsert(payload,{onConflict:"event_id,member_id"});if(saveError){button.disabled=false;button.textContent=old;alert(saveError.message?.includes("locked")?"Tee times have already been produced. Please contact an admin to make a change.":"We couldn’t save your RSVP. Please try again.");return;}await loadEvents();
    }));
