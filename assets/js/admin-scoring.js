(() => {
  ...
  const status=message=>["adminLiveScoringStatus","adminFinishRoundStatus","adminGroupScorecardStatus"].forEach(id=>{const el=$(id);if(el)el.textContent=message;});
  ...
  const options=(items,prompt)=>`<option value="">${prompt}</option>${items.map(item=>`<option value="${esc(item.id)}">${esc(item.name)}${item.tee_sets?` — ${Math.max(0,...(item.tee_sets||[]).map(tee=>tee.holes?.length||0))} holes`:item.county?` · ${esc(item.county)}`:""}</option>`).join("")}`;
  ...
  $("adminGolfClub")?.addEventListener("change",async event=>{try{$("adminGolfCourse").innerHTML='<option value="">Select the course being played</option>';$('adminYellowTee').innerHTML='<option value="">Select course first</option>';$('adminRedTee').innerHTML='<option value="">Select course first</option>';$('adminImportScorecard').disabled=true;await loadClubCourses(event.target.value);}catch(error){status(error.message);}});
  ...
  $("adminImportScorecard")?.addEventListener("click",async()=>{
    ...
    const event=events.find(item=>item.id===activeEventId),course=$('adminGolfCourse').selectedOptions[0],layoutName=(course?.textContent||event?.name).replace(/ — \d+ holes$/,"").trim(),clubName=$('adminGolfClub').selectedOptions[0]?.textContent?.replace(/ · .*$/,"").trim()||event?.name;const holeResult=await client.from("event_holes").upsert(holes,{onConflict:"event_id,hole_number"});
    if(holeResult.error)return status(holeResult.error.message);try{await window.BarfordCourseLayouts.saveLayout({eventId:activeEventId,clubName,layoutName,externalClubId:$("adminGolfClub").value,externalCourseId:$("adminGolfCourse").value,men:yellow,women:red});}catch(error){return status(error.message)}await loadEvent(activeEventId);saved(`${layoutName} saved with ${yellow.name} and ${red.name} tees.`);status(`${layoutName} loaded with ${yellow.name} and ${red.name} tees. Publish tee times first, then create group scorecards when you are ready.`);
  });
