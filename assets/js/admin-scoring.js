(() => {
  ...
  $("adminFindCourseData")?.addEventListener("click",async()=>{try{const event=events.find(item=>item.id===activeEventId);status("Matching the event location with UK Golf API…");const data=await invokeCourse({action:"nearby",latitude:event?.latitude,longitude:event?.longitude});$("adminGolfClub").innerHTML=options(data.clubs||[],"Select the linked golf club");$("adminGolfClub").disabled=false;if(data.clubs?.length===1){$("adminGolfClub").value=data.clubs[0].id;await loadClubCourses(data.clubs[0].id);}else status("Select the correct golf club.");}catch(error){status(error.message);}});
