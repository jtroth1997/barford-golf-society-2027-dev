(() => {
  ...
  $("adminSaveHoles")?.addEventListener("click",async()=>{
    ...
    status("Saving course card…");const {error}=await client.from("event_holes").upsert(holes,{onConflict:"event_id,hole_number"});if(error)return status(error.message);let prepared=false;try{prepared=await maybeAutoPrepare();}catch(autoError){return status(autoError.message);}await loadEvent(activeEventId);saved("Course and tee information saved successfully.");status(prepared?"Course saved and group scorecards prepared automatically.":"Course card saved. Scorecards will prepare automatically when tee times are published.");
  });
  ...
  $("adminImportScorecard")?.addEventListener("click",async()=>{
    ...
    if(holeResult.error||eventResult.error)return status((holeResult.error||eventResult.error).message);let prepared=false;try{prepared=await maybeAutoPrepare();}catch(autoError){return status(autoError.message);}await loadEvent(activeEventId);saved(`${course?.textContent||"Course"} saved with ${yellow.name} and ${red.name} tees.`);status(`${course?.textContent||"Course"} loaded with ${yellow.name} and ${red.name} tees.${prepared?" Group scorecards are ready.":" Cards will prepare when tee times are published."}`);
  });
  ...
  $("adminCompleteRound")?.addEventListener("click",async()=>{
    ...
    const {error}=await client.rpc("complete_event_round",{target_event_id:completedEventId});if(error){status(error.message);button.disabled=false;button.textContent="Finish round and publish results";return;}
