(() => {
  ...
  $("adminSaveHoles")?.addEventListener("click",async()=>{
    ...
    status("Saving course card…");const {error}=await client.from("event_holes").upsert(holes,{onConflict:"event_id,hole_number"});if(error)return status(error.message);await loadEvent(activeEventId);saved("Course and tee information saved successfully.");status("Course card saved. Publish tee times first, then create group scorecards when you are ready.");
  });
  ...
  $("adminImportScorecard")?.addEventListener("click",async()=>{
    ...
    if(holeResult.error||eventResult.error)return status((holeResult.error||eventResult.error).message);await loadEvent(activeEventId);saved(`${course?.textContent||"Course"} saved with ${yellow.name} and ${red.name} tees.`);status(`${course?.textContent||"Course"} loaded with ${yellow.name} and ${red.name} tees. Publish tee times first, then create group scorecards when you are ready.`);
  });
